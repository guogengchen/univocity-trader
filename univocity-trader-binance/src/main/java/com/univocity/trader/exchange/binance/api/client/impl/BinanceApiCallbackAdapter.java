...
/**
 * An adapter/wrapper which transforms a Callback from Retrofit into a BinanceApiCallback which is exposed to the
 * client.
 */
    ...
    public void onResponse(Call<T> call, Response<T> response) {
        if (response.isSuccessful()) {
            ...
            if (response.code() == 504) {
                // HTTP 504 return code is used when the API successfully sent the message but not get a response within
                // the timeout period.
                // It is important to NOT treat this as a failure; the execution status is UNKNOWN and could have been a
                // success.
                return;
            }
            try {
