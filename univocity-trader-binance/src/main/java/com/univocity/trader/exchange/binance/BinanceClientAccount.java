...
class BinanceClientAccount implements ClientAccount {
    ...
    public OrderBook getOrderBook(String symbol, int depth) {
        com.univocity.trader.exchange.binance.api.client.domain.market.OrderBook book =
            client.getOrderBook(symbol, depth == 0 ? 5 : depth);
    ...
    public void cancel(Order order) {
        CancelOrderResponse response =
            client.cancelOrder(new CancelOrderRequest(order.getSymbol(), Long.valueOf(order.getOrderId())));
