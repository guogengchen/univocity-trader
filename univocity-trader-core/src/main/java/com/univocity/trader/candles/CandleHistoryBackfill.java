...
public class CandleHistoryBackfill {
    ...
    private Instant resume(String symbol) {
        ...
        var q =
            "SELECT max(close_time) FROM candle WHERE symbol = ? AND ts = (SELECT max(ts) FROM candle WHERE symbol = ? LIMIT 1)";
        Number closeOfLatestCandleLoaded =
            candleRepository.db().queryForObject(q, new Object[] {symbol, symbol}, Number.class);
    ...
    public <T> void fillHistoryGaps(Exchange<T, ?> exchange, String symbol, Instant from, Instant to,
        ...
        if (!gaps.isEmpty()) {
            // if (gaps.size() > 30) {
            // log.warn("Too many gaps in history: {}. Will process the 30 most recent and ignore older transactions",
            // gaps.size());
        ...
        for (long[] gap : gaps) {
            ...
            try {
                ...
                if (count <= 2 && exchange.historicalCandleCountLimit() > 0) {
                    noDataCount++;
                    // log.info("No Candles found for {} between {} and {}", symbol,
