...
/**
 * A group of {@link Indicator} used to assist in creating a {@link Strategy} efficiently. All indicators in an
 * {@link IndicatorGroup} share
 * a common {@link Aggregator} which is responsible for merging higher resolution candles into a lower resolution
 * {@link Candle}
 * (e.g. merging five 1-minute candles into a single 5-minutes candle) so any indicator of the group that works in the
 * same time frame
...
public abstract class IndicatorGroup {
    ...
    /**
     * Initializes all indicators of this group (returned via {@link #getAllIndicators()}) to use the same
     * {@link Aggregator}
     *
    ...
    /**
     * Cycles through all indicators in this group and invokes their {@link Indicator#accumulate(Candle)} method, so
     * they can update their state.
     *
     * After all indicators are updated, method {@link #candleAccumulated(Candle)} will be invoked to notify
    ...
    /**
     * Callback method used to notify subclasses that a {@link Candle} was accumulated and the indicators of this group
     * might have a new state.
