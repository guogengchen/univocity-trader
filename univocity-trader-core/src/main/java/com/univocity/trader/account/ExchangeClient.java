...
public class ExchangeClient<T> implements Client {
    ...
    public void initialize(CandleRepository candleRepository, Exchange<T, ?> exchange, SmtpMailSender mailSender) {
        ...
        final SymbolPriceDetails priceDetails =
            new SymbolPriceDetails(exchange, accountManager.getReferenceCurrencySymbol()); // loads price information
                                                                                           // from exchange
        ...
        for (Map.Entry<String, String[]> e : accountManager.configuration().symbolPairs().entrySet()) {
            ...
            TradingManager tradingManager =
                new TradingManager(exchange, priceDetails, accountManager, assetSymbol, fundSymbol, Parameters.NULL);
            if (root == null) {
                root = tradingManager;
            }
