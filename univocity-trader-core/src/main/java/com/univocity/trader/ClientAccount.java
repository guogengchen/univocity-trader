 ...
 * their own implementation to all or some symbols using
 * {@link AccountConfiguration#orderManager(OrderManager, String...)}.
 *
 * Note that the {@link DefaultOrderManager} will keep orders open for up to 10 minutes by default, and will cancel the
 * order
...
public interface ClientAccount {
     ...
     *
     * The order status will be updated by polling the state at a fixed interval defined by
     * {@link OrderManager#getOrderUpdateFrequency).
     *
     * Once the order reaches status {@code FILLED} or {@code CANCELLED}, {@link OrderManager#finalized(Order)} will be
     * called
     * so its implementation can perform any cleanup necessary.
     *
     * If an order is updated (i.e. it is still open and some quantity was filled), {@link OrderManager#updated(Order)}
     * will be called.
     *
     * If no changes occurred after polling the exchange for an update on the status of the order,
     * {@link OrderManager#unchanged(Order)}
     * will be called.
     *
     * An {@link Order} can be cancelled at any time after from the {@link OrderManager} by invoking
     * {@link Order#cancel()}
     *
    ...
    /**
     * Updates the balances of all symbols traded by this account and returns them in a map of symbol to {@link Balance}
     * instances, which
    ...
    /**
     * Returns the {@link TradingFees} applied to this account as determined by the exchange. Even though the exchange
     * generally
     * subtracts the amount from the account directly, for live trading it's best to provide a worst case estimate so
     * that
    ...
    default TradingFees getTradingFees() {
        ...
        // to return the exchange fees to correctly calculate quantities and amounts for orders when balances get close
        // to
     ...
     * Used by {@link OrderManager#prepareOrder(SymbolPriceDetails, OrderBook, OrderRequest, Candle)} to update the unit
     * price and quantity
     * to buy/sell in an initial {@link OrderRequest} which is always generated based on the latest close price of the
     * symbol, and the current
     * account free balance.
     *
     * Called whenever {@link AccountManager#buy(String, String, double)} or
     * {@link AccountManager#sell(String, String, double)} is invoked, then
     * the returned {@link OrderBook} will be sent for the {@link OrderManager} associated with the given symbol to
     * prepare an order for execution.
     *
     * Note that unless the user provides their own {@link OrderManager},
     * the {@link DefaultOrderManager} will use the {@link OrderBook} to calculate the central price point in the spread
     * between
    ...
    /**
     * Updates the status of a given {@link Order}. Used by the {@link AccountManager} to poll the exchange at the
     * interval specified
     ...
     *
     * @return the updated order to be sent to the {@link OrderManager} via {@link OrderManager#unchanged(Order)},
     *         {@link OrderManager#updated(Order)} or
     ...
     *
     * @return {@code true} if any order of the given symbol was finalized (i.e. FILLED or CANCELLED) by this method,
    ...
    /**
     * Identifies whether this account is a simulated account or a live one connected to a live exchange with real
     * funds.
     *
     ...
     * order, in addition to the required margin reserve percentage returned by this method (which will lock the
     * corresponding
