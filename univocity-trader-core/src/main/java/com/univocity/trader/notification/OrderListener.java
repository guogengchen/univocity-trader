...
public interface OrderListener {
    ...
    /**
     * Notification of a new {@link Order} submission to the exchange. You must override
     * {@link #orderFinalized(Order, Trade, Client)} to
     ...
     * @param trade  the object responsible to track a trade from the first order creation, and which contains many
     *               details
     *               regarding the current symbol, such as {@link Trade#lastClosingPrice()} and
     *               {@link Trade#latestCandle()};
     *               and the trade itself, e.g. {@link Trade#averagePrice()}, {@link Trade#minPrice()},
     *               {@link Trade#ticks()}, etc.
     *               Might be {@code null}.
    ...
    /**
     * Notification that an order already submitted to the exchange is finalized (i.e. either {@code FILLED} or
     * {@code CANCELLED}).
     * Notice that a cancelled order might have been partially filled to some point, so check if
     * {@link Order#getFillPct()} is
