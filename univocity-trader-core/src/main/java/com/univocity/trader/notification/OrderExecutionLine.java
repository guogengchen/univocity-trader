    ...
    public OrderExecutionLine(Order order, Trade trade, Trader trader, Client client) {
        ...
        if (order != null) {
            ...
            if (orderType == Order.Type.MARKET && fillPct == 0.0) {
                orderAmount =
                    amountDetails.priceToString(order.getQuantity().doubleValue() * trader.lastClosingPrice());
            } else {
        ...
        if (trade != null) {
            ...
            if (trade.isFinalized()) {
                ...
                profitLossReferenceCurrency =
                    refPriceDetails.priceToString(trade.actualProfitLossInReferenceCurrency());
            }
