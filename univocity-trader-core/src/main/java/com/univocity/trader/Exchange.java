 ...
 *            {@link #generateCandle(Object)} for in-memory calculations and {@link PreciseCandle} with
 *            {@link #generatePreciseCandle(Object)}
 *            for storage.
 *
 * @see Candle
 * @see PreciseCandle
...
public interface Exchange<T, C extends AccountConfiguration<C>> {
    ...
    /**
     * Provides the latest exchange-specific candles/ticks for a given symbol at the given time interval. This method is
     * intended to return
     * quickly so the list returned should not contain much more than the price information of the last 30 minutes.
     *
     * @param symbol   the symbol whose latest price information should be returned from the exchange (e.g. BTCUSDT,
     *                 MSFT, etc)
     * @param interval the duration of each candle. e.g. if {@code TimeInterval.minutes(5)} should ideally return the
     *                 latest 5 minute candle
     *                 (typically with open and close time, open and close prices, highest and lowest prices, and traded
     *                 volume).
     *                 Not every bit of information is required, but at least the closing price and the close time
     *                 should be provided.
     *
    ...
    /**
     * Returns historical candles/ticks for a given symbol at the given time interval, for a given time period. Used to
     * populate
     * the local database managed by {@link CandleRepository} for backtesting and {@link Strategy} development. If the
     * live exchange
     ...
     *
     * Method {@link CandleRepository#fillHistoryGaps(Exchange, String, Instant, TimeInterval)} will cycle through the
     * gaps in history and fill them accordingly
     ...
     * @param interval  the duration of each candle. e.g. if {@code TimeInterval.minutes(5)} should ideally return the
     *                  latest 5 minute candle
     *                  (typically with open and close time, open and close prices, highest and lowest prices, and
     *                  traded volume).
     *                  Not every bit of information is required, but at least the closing price and the close time
     *                  should be provided.
     ...
     *
     * @return candles with historical {@code Exchange}-specific candles/ticks, for the given period (or less), which
     *         will be converted to a {@link Candle} via
     *         {@link #generateCandle(Object)} for in-memory calculations and {@link PreciseCandle} with
     *         {@link #generatePreciseCandle(Object)}
     *         for storage.
     */
    ...
    /**
     * Converts an {@code Exchange}-specific candle/tick to a {@link Candle} that is used by the framework for fast
     * in-memory calculations.
     *
     * At least the closing price and open/close times in the returned {@link Candle} should be populated.
     *
     * @param exchangeCandle the {@code Exchange}-specific candle/tick details whose data need to be converted into a
     *                       {@link Candle}.
     *
    ...
    /**
     * Converts an {@code Exchange}-specific candle/tick to a {@link PreciseCandle} that is used by the framework for
     * database storage with
     * no precision loss.
     *
     * At least the closing price and open/close times in the returned {@link PreciseCandle} should be populated. Set
     * the fields of
     * the {@link PreciseCandle} to {@code BigDecimal.ZERO} if the input candle does not have the corresponding data and
     * it can't be derived.
     *
     * @param exchangeCandle the {@code Exchange}-specific candle/tick details whose data need to be converted into a
     *                       {@link PreciseCandle}.
    ...
    /**
     * Connects to the live exchange stream to receive real time signals which will be delegated to a given
     * {@link TickConsumer}.
     *
     * On top of the live stream, the {@link LiveTrader} will continuously check for updates on the signals of the
     * symbols subscribed to with this method.
     * If the {@link LiveTrader} does not receive price updates within the given {@code tickInterval}, symbol prices
     * will be polled using
     * {@link #getLatestTick(String, TimeInterval)}.
     *
     * @param symbols      a comma separated list of symbols to subscribe to.
     * @param tickInterval the frequency of the signals to be received such as every 1 minute, 1 hour, 5 seconds, etc
     *                     (whichever is supported by the exchange)
     * @param consumer     a consumer of {@code Exchange}-specific candle/tick details whose data need to be converted
     *                     into a {@link Candle} and then submitted
     *                     for further processing (i.e. {@link Strategy} analysis, {@link Signal} generation and
     *                     potential trading by {@link ExchangeClient})
     */
    ...
    /**
     * Quick price update on all available symbols. Used for calculating the current account position, available funds
     * for trading
     * and (in {@link AccountManager#allocateFunds(String)} and sending e-mail notifications with the current net worth
     * of the account.
     *
    ...
    /**
     * Returns the details about each traded symbol, such as minimum order size and decimal places to use for correct
     * formatting and proper order
     * generation (some exchanges may not accept prices such as {@code 14.44321} and would require {@code 14.4432}
     * instead).
     *
     * If not applicable, simply return {@code null}
     *
     * @return a map with the {@link SymbolInformation} associated with all available symbols in this exchange, if
     *         applicable.
     */
     ...
     * @param fundSymbol  symbol of the funds used to represent the price of the given asset (ETH, USD, BRL). The
     *                    framework will always send a symbol here, if
     *                    no trading pair exists, the {@code referenceCurrency} provided by
     *                    {@link AccountConfiguration#referenceCurrency()} will be sent.
     *                    It can be ignored if not applicable (when trading stocks for example, and the account only
     *                    buys stocks with USD)
     *
    ...
    /**
     * Opens a client account with the given {@code apiKey} and {@code secret}, which authorizes the
     * {@link ExchangeClient} to trade and update their account balance.
     *
     * @param accountConfiguration the configuration of a client account (typically with API key and secret) to be used
     *                             to connect to the desired account.
     *
     * @return an exchange-specific implementation of the {@link ClientAccount} interface, which allows the
     *         {@link ExchangeClient} to trade through this framework.
     */
     ...
     * @param symbol   the symbol whose latest price information should be returned from the exchange (e.g. BTCUSDT,
     *                 MSFT, etc)
     * @param interval the duration of the candle. e.g. if {@code TimeInterval.minutes(5)} should ideally return the
     *                 latest 5 minute candle
     *                 (typically with open and close time, open and close prices, highest and lowest prices, and traded
     *                 volume).
     *                 Not every bit of information is required, but at least the closing price and the close time
    ...
    /**
     * Handles errors from the exchange server that might be produced when polling for latest prices in case the live
     * stream becomes unavailable, slow or
     * unreliable.
     *
     * The {@link LiveTrader} will continuously check for updates on the signals of the symbols subscribed to with
     * {@link #openLiveStream(String, TimeInterval, TickConsumer)}. If no updates are received within the given
     * {@link TimeInterval}, the exchange will be
     * polled using {@link #getLatestTick(String, TimeInterval)} to ensure the signals generated by the
     * {@link Strategy}s in use are reliable.
     *
     * @param symbol the symbol whose latest candle/tick information did not come through the live stream, and was
     *               polled.
