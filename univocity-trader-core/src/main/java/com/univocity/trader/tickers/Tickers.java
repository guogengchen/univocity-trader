...
public class Tickers {
    ...
    private void read() {
        ...
        for (Ticker ticker : new CsvRoutines(settings).iterate(Ticker.class,
            Tickers.class.getResourceAsStream(DATAFILE_NAME), "UTF-8")) {
    ...
    private String getSymbol(String name) {
        ...
        if (currency == null) {
            throw new IllegalArgumentException(
                "Unknown ticker: " + name + " . Available tickers are: " + new TreeSet<>(tickers.keySet()));
        }
