    ...
    public static <T> T[] getInstances(String symbol, Parameters params, InstancesProvider<T> provider,
        ...
        for (T instance : instancesToUse) {
            if (allInstances.contains(instance)) {
                throw new IllegalStateException(
                    "Can't execute market simulation. " + description + " instance provided for symbol " + symbol
                        + " is already in use. Make sure to build a *new* " + description + " object for each symbol.");
            } else {
