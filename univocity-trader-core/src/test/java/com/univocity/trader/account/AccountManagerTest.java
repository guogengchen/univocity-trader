...
public class AccountManagerTest {
    ...
    private AccountManager getAccountManager() {
        ...
        TradingManager m =
            new TradingManager(new SimulatedExchange(account), null, account, "ADA", "USDT", Parameters.NULL);
    ...
    private void checkProfitLoss(Trade trade, double initialBalance, double totalInvested) {
        ...
        double invested =
            totalInvested + trader.tradingFees().feesOnAmount(totalInvested, Order.Type.LIMIT, Order.Side.SELL);
    ...
    public void testShortPositionTrading() {
        ...
        double averagePrice =
            getInvestmentAmount(trader, ((quantity1 * 0.9) + (quantity2 * 1.2))) / (quantity1 + quantity2);
        assertEquals(averagePrice, trade.averagePrice(), 0.001);
    ...
    public void testShortPositionTradingNoMax() {
        ...
        Trade trade = trader.trades().iterator().next();
        double quantity1 =
            checkTradeAfterShortSell(usdBalance, reservedBalance, trade, initialBalance, 0.0, 0.9, 0.9, 0.9);
